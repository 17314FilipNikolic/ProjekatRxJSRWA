...
export class OrderService{
    ...
    createAdObservable(ads: string[]){
        this.adObservable = this.adService.handleButtonClick(ads);
    }

    makeOrder(order: OrderView, host: HTMLElement){
        merge(
            this.foodObservable.pipe(
                map((food) => order.setFoodOrder(food)),
            ),
            this.drinkObservable.pipe(
                map((drink) => order.setDrinkOrder(drink))
            ),
            this.adObservable.pipe(
                map((ads) => order.setAdsOrder(ads))
            )
